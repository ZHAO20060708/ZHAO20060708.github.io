...
## 例1：证明
$ \bm{\lim_{n\to\infty}\frac{n+(-1)^{n-1}}n=1.} $
...
## 例2 设$ \bm{x_n\equiv C} $（$ \bm{C} $为常数），证明
$ \bm{\lim_{n\to\infty} x_n=C.} $

证：  
&emsp;&emsp;任给$ \varepsilon>0 $,对于一切正整数$ n $,  
&emsp;&emsp;$ |x_n-C|=|C-C|=0<\varepsilon $成立，  
&emsp;&emsp;所以，$ \lim_{n\to\infty} x_n=C. $  
&emsp;&emsp;*明：常数列的极限等于同一常数。 

_**小结：用定义证明数列极限存在时，关键是任意给定$ \bm{\varepsilon>0} $,寻找$ N $,但不必要求最小的$ \bm{N} $.**_
...
根据复合函数求导法则，$ \frac{d}{dx} f(u) =
f^\prime(u) \cdot u^\prime $
